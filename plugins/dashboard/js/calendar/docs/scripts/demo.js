...
app.controller('calendarDashBoardCtrl', function ($scope, $rootScope, $location, Data, cssInjector,$http, $modal, cssInjector, moment) { 
    ...
    Data.get('proposals').then(function(data){
   	 ...
   	 console.log($scope.proposalsList);
   	 $scope.proposalsList.forEach(function(value,i){
   		 if(value.dueOn != ""){
   			 console.log(new Date(value.dueOn));
   			 /*$scope.events.push({title: 'Proposals due on date',start: new Date(value.dueOn),url: '#/calendarDashboard/'+value.dueOn});*/
   			$scope.events.push({
   		        title: 'Event 1',
   		        type: 'warning',
   		        startsAt: new Date(value.dueOn),
   		        endsAt:new Date(value.dueOn)
   		      });
   		 }
